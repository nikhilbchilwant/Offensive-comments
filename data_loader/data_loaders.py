...
class JigsawDataLoader(BaseDataLoader):
    ...
    def __init__(self, data_dir, batch_size, tokenizer_name, shuffle=False, validation_split=0.0, num_workers=1, data_red_factor=1):
        ...
        # train_dataset = getattr(dataset_module, dataset_name)(train_labels, train_comments, tokenizer)
        # val_dataset = getattr(dataset_module, dataset_name)(val_labels, val_comments, tokenizer)


    def get_train_dataloader(self):
      train_labels = self.train_data['toxic_label_max']
      train_comments = self.train_data['comment_text']
      ...
      train_dataset = Toxic_Dataset(train_labels, train_comments, self.tokenizer)
    ...
    def get_val_dataloader(self):
      val_labels = self.validate_data['toxic_label_max']
      val_comments = self.validate_data['comment_text']
      val_labels, val_comments = val_labels.to_list(), val_comments.to_list()

      val_dataset = Toxic_Dataset(val_labels, val_comments, self.tokenizer)
